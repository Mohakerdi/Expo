// ==================== Shader.h (النسخة النهائية - للتطبيق بعد حل مشكلة المساحة) ====================
#pragma once
#include "core.h"
#include <string>
#include <unordered_map>
#include <glm/glm.hpp> // Assuming you need glm for matrix/vector uniforms

struct ShaderSource {
	std::string vertexSource;
	std::string FragmentSource;
};

class Shader
{
private:
	unsigned int m_RendererID;
	std::string m_FilePath; // for debug
	// caching for uniforms
	std::unordered_map<std::string, int> m_UniformLoactionCache;

public:
	Shader(const std::string& filepath);
	Shader(const std::string& vertexPath, const std::string& fragmentPath);
	~Shader();

	inline std::string getPath() const { return m_FilePath; }

	void Bind() const;
	void Unbind() const;

	void SetUniform1i(const std::string& name, int value);
	void SetUniform1f(const std::string& name, float value);

	void SetUniform3f(const std::string& name, float v0, float v1, float v2);
	// CORRECTED: Removed 'const&' to match your original .cpp file
	void SetUniform3fv(const std::string& name, glm::vec3 vec);

	void SetUniform4f(const std::string& name, float v0, float v1, float v2, float v3);
	// CORRECTED: Removed 'const&' to match your original .cpp file
	void SetUniform4fv(const std::string& name, glm::vec4 vec);

	void setUniformMat4f(const std::string& name, const glm::mat4& matrix);

private:
	unsigned int CompileShader(unsigned int type, const std::string& source);
	unsigned int CreateShader(const std::string& vertexShader, const std::string& fragmentShader);
	ShaderSource ParseShader(const std::string& filepath) const;
	int getUniformLoacation(const std::string& name); // Keep the typo for now
	std::string readShader(const std::string& path) const;
};

